...
def sync_detailed(
    ...
    * If the build process is successful, the `data` field in the closing-message contains the
    `image_id`.
    * If the build process fails the closing message's `data` field contains the latest snapshot or is
    set to `\"\"` if the build failed before any snapshots have been created.
