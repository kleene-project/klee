import os
import json
from testutils import run, inspect, create_dockerfile, create_deployment_file

# pylint: disable=unused-argument


def test_existing_and_correct_image(cleanup):
    deployment = """
---
images:
- tag: "FreeBSD:latest"
...
def test_image_created_with_different_method_zfsclone(cleanup):
    deployment = """
---
images:
- tag: "FreeBSD:latest"
...
def test_image_created_with_different_method_fetch(cleanup):
    deployment = """
---
images:
- tag: "FreeBSD:latest"
...
def test_image_built_instead_of_created(cleanup):
    deployment = """
---
images:
- tag: "FreeBSD:latest"
...
"""
    ...
    run(
        "container create -m teststorage:/mnt/storage -m /usr/local/etc:/mnt/test_etc --name testcon FreeBSD:latest"
    )
    assert _deploy_diff(deployment) == _diff_result()
    run("rmc testcon")
    run("rmv teststorage")
...
"""
    run("volume create teststorage")
    run(
        "container create -m /usr/local/etc:/mnt/test_etc --name testcon FreeBSD:latest"
    )
    assert _deploy_diff(deployment) == _diff_result(
        containers={
            "testcon": [
...
def test_diff_container_with_mounts_having_different_readonly(
    testimage_and_cleanup,
):
    deployment = """
---
    ...
    assert _deploy_diff(deployment) == _diff_result(
        {
            "testcon": [
                {
                    "host_source": "nullfs:/usr/local/etc",
                    "spec_source": "nullfs:/usr/local/etc1",
...
"""
    ...
    assert _deploy_diff(deployment) == _diff_result(
        containers={
            "testcon": [
                {
                    "host_destination": "/mnt/test_etc",
                    "spec_destination": "/mnt/new_test_etc",
